...
// Re-export factory helpers that match the old free-function API for migration paths.
export function createInitialState(options: StartRunOptions): RunState {
  return Run.start(options).toSnapshot();
}
...
export function markStageCompleted(
  ...
  run.completeStage(stage, nextStage, options);
  return run.toSnapshot();
}

export function markStageSkipped(state: RunState, _stage: StageName, nextStage: NextStage): RunState {
  const run = Run.rehydrate(state);
  run.setNextStage(nextStage);
  return run.toSnapshot();
}

export function advancePhase(state: RunState, totalPhases: number): RunState {
  const run = Run.rehydrate(state);
  run.advancePhase(totalPhases);
  return run.toSnapshot();
}
