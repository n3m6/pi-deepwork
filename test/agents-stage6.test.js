...
const taskSpecReviewerPath = path.join(agentsDir, 'qrspi-task-spec-reviewer.md');
...
const REQUIRED_FIELDS = ['description', 'tools', 'model', 'thinking', 'max_turns', 'prompt_mode', 'extensions', 'enabled'];
...
test('qrspi-plan-writer.md file exists and has parseable frontmatter', () => {
  assert.ok(planWriterFM !== null, 'qrspi-plan-writer.md has parseable frontmatter');
});

test('qrspi-task-spec-writer.md file exists and has parseable frontmatter', () => {
  assert.ok(taskSpecWriterFM !== null, 'qrspi-task-spec-writer.md has parseable frontmatter');
});

test('qrspi-task-spec-reviewer.md file exists and has parseable frontmatter', () => {
  assert.ok(taskSpecReviewerFM !== null, 'qrspi-task-spec-reviewer.md has parseable frontmatter');
});

test('qrspi-plan-reviewer.md file exists and has parseable frontmatter', () => {
  assert.ok(planReviewerFM !== null, 'qrspi-plan-reviewer.md has parseable frontmatter');
});

test('qrspi-baseline-checker.md file exists and has parseable frontmatter', () => {
  assert.ok(baselineCheckerFM !== null, 'qrspi-baseline-checker.md has parseable frontmatter');
});
...
test('qrspi-plan.md frontmatter — tools field', () => {
  assert.equal(planFM.tools, 'read, bash, grep, find, ls, write, edit, qrspi_dispatch');
});
...
test('All 6 agents — thinking field is low', () => {
  ...
  assert.equal(planWriterFM.thinking, 'low', 'qrspi-plan-writer thinking must be low');
  assert.equal(taskSpecWriterFM.thinking, 'low', 'qrspi-task-spec-writer thinking must be low');
  assert.equal(taskSpecReviewerFM.thinking, 'low', 'qrspi-task-spec-reviewer thinking must be low');
  assert.equal(planReviewerFM.thinking, 'low', 'qrspi-plan-reviewer thinking must be low');
  assert.equal(baselineCheckerFM.thinking, 'low', 'qrspi-baseline-checker thinking must be low');
});

test('All 6 agents — extensions field is false', () => {
  assert.equal(planFM.extensions, 'false', 'qrspi-plan extensions must be false');
  assert.equal(planWriterFM.extensions, 'false', 'qrspi-plan-writer extensions must be false');
  assert.equal(taskSpecWriterFM.extensions, 'false', 'qrspi-task-spec-writer extensions must be false');
  assert.equal(taskSpecReviewerFM.extensions, 'false', 'qrspi-task-spec-reviewer extensions must be false');
  assert.equal(planReviewerFM.extensions, 'false', 'qrspi-plan-reviewer extensions must be false');
  assert.equal(baselineCheckerFM.extensions, 'false', 'qrspi-baseline-checker extensions must be false');
});

test('All 6 agents — enabled field is false', () => {
  assert.equal(planFM.enabled, 'false', 'qrspi-plan enabled must be false');
  assert.equal(planWriterFM.enabled, 'false', 'qrspi-plan-writer enabled must be false');
  assert.equal(taskSpecWriterFM.enabled, 'false', 'qrspi-task-spec-writer enabled must be false');
  assert.equal(taskSpecReviewerFM.enabled, 'false', 'qrspi-task-spec-reviewer enabled must be false');
  assert.equal(planReviewerFM.enabled, 'false', 'qrspi-plan-reviewer enabled must be false');
  assert.equal(baselineCheckerFM.enabled, 'false', 'qrspi-baseline-checker enabled must be false');
});

test('All 6 agents — prompt_mode field is replace', () => {
  assert.equal(planFM.prompt_mode, 'replace', 'qrspi-plan prompt_mode must be replace');
  assert.equal(planWriterFM.prompt_mode, 'replace', 'qrspi-plan-writer prompt_mode must be replace');
  assert.equal(taskSpecWriterFM.prompt_mode, 'replace', 'qrspi-task-spec-writer prompt_mode must be replace');
  assert.equal(taskSpecReviewerFM.prompt_mode, 'replace', 'qrspi-task-spec-reviewer prompt_mode must be replace');
  assert.equal(planReviewerFM.prompt_mode, 'replace', 'qrspi-plan-reviewer prompt_mode must be replace');
  assert.equal(baselineCheckerFM.prompt_mode, 'replace', 'qrspi-baseline-checker prompt_mode must be replace');
});
...
test('qrspi-plan.md body — uses qrspi_dispatch (not task) for subagent dispatch', () => {
  assert.ok(planBody.includes('qrspi_dispatch'), 'body must contain qrspi_dispatch');
  assert.ok(!planBody.includes('the task tool'), 'body must not contain "the task tool"');
  assert.ok(!planBody.includes('Invoke'), 'body must not contain "Invoke" as dispatch reference');
});
...
test('qrspi-plan.md body — uses Read not cat for artifact reads', () => {
  assert.ok(planBody.includes('Read .pipeline/'), 'body must contain "Read .pipeline/"');
  assert.ok(!planBody.includes('cat .pipeline/'), 'body must not contain "cat .pipeline/"');
});

test('qrspi-plan.md body — contains bash mkdir commands for pipeline directories', () => {
  assert.ok(planBody.includes('bash: mkdir -p .pipeline/'), 'body must contain bash mkdir command');
});

test('qrspi-plan.md body — does not contain @build subagent reference', () => {
  assert.ok(!planBody.includes('@build'), 'body must not contain @build subagent reference');
});
...
test('qrspi-plan.md body — mentions quick-fix and exactly one task constraint', () => {
  ...
  assert.ok(
    planBody.includes('exactly one task') || planBody.includes('exactly one outline') || planBody.includes('single task'),
...
test('qrspi-plan.md body — contains instructions for omitting DESIGN and STRUCTURE on quick-fix', () => {
  assert.ok(planBody.includes('quick-fix') && planBody.includes('DESIGN'),
    'body must reference quick-fix and DESIGN omission');
  assert.ok(planBody.includes('quick-fix') && planBody.includes('STRUCTURE'),
...
test('qrspi-plan.md body — contains review_round with 6-round cap', () => {
  assert.ok(planBody.includes('review_round'), 'body must reference review_round');
  assert.ok(planBody.includes('review_round = 6'), 'body must reference review_round = 6 cap');
  assert.ok(planBody.includes('review_round < 6'), 'body must reference review_round < 6 loop condition');
});

test('qrspi-plan.md body — contains stable-cap detection logic', () => {
  assert.ok(planBody.includes('stable-cap'), 'body must reference stable-cap terminal state');
  assert.ok(planBody.includes('Fix Guidance'), 'body must reference Fix Guidance for stable-cap detection');
});

test('qrspi-plan.md body — contains unclean-cap terminal state', () => {
  assert.ok(planBody.includes('unclean-cap'), 'body must reference unclean-cap terminal state');
});
...
test('qrspi-plan.md body — three-way decision: PASS stop, FAIL+<6 retry, FAIL+6 unclean-cap', () => {
  ...
  assert.ok(planBody.includes('unclean-cap'), 'body must reference unclean-cap state');
  assert.ok(
...
test('qrspi-plan.md body — contains Status — PASS and Status — FAIL return format', () => {
  assert.ok(planBody.includes('### Status — PASS'), 'body must contain PASS return format');
  assert.ok(planBody.includes('### Status — FAIL'), 'body must contain FAIL return format');
});

test('qrspi-plan.md body — contains Files Written, Summary, and Telemetry return fields', () => {
  assert.ok(planBody.includes('### Files Written'), 'body must contain Files Written field');
  assert.ok(planBody.includes('### Summary'), 'body must contain Summary field');
  assert.ok(planBody.includes('### Telemetry'), 'body must contain Telemetry field');
});
...
test('qrspi-plan.md body — references quality gate with hard-fail conditions', () => {
  assert.ok(planBody.includes('Quality Gate'), 'body must reference Quality Gate');
  assert.ok(planBody.includes('not addressed'), 'body must reference missing AC coverage');
  assert.ok(planBody.includes('depends on a later task'), 'body must reference forward dependencies');
  assert.ok(planBody.includes('placeholders'), 'body must reference placeholders');
  assert.ok(planBody.includes('quick-fix') && planBody.includes('more than one task'), 'body must reference quick-fix cardinality in quality gate');
});
...
test('qrspi-plan-writer.md body — contains ### plan.md output section', () => {
  assert.ok(planWriterBody.includes('### plan.md'), 'body must contain ### plan.md output section');
});

test('qrspi-plan-writer.md body — contains ### phase-manifest.md output section', () => {
  assert.ok(planWriterBody.includes('### phase-manifest.md'), 'body must contain ### phase-manifest.md output section');
});

test('qrspi-plan-writer.md body — contains ### task-NN.outline output section', () => {
  assert.ok(planWriterBody.includes('### task-NN.outline') || planWriterBody.includes('### task-'), 'body must contain task outline output section');
});

test('qrspi-plan-writer.md body — uses artifact-section output contract, not Status-based return format', () => {
  assert.ok(
    !planWriterBody.includes('### Status — PASS') && !planWriterBody.includes('### Status — FAIL'),
...
test('qrspi-plan-writer.md body — contains No placeholders rule', () => {
  assert.ok(planWriterBody.includes('No placeholders'), 'body must contain No placeholders rule');
});

test('qrspi-plan-writer.md body — contains Coverage Notes are complete rule', () => {
  assert.ok(planWriterBody.includes('Coverage Notes are complete'), 'body must contain Coverage Notes are complete rule');
});

test('qrspi-plan-writer.md body — contains Quick-fix cardinality exactly one task rule', () => {
  assert.ok(planWriterBody.includes('Quick-fix cardinality') || planWriterBody.includes('exactly one task outline'),
    'body must contain quick-fix cardinality rule');
});

test('qrspi-plan-writer.md body — contains Completed phases are immutable rule', () => {
  assert.ok(planWriterBody.includes('Completed phases are immutable'), 'body must contain Completed phases are immutable rule');
});

test('qrspi-plan-writer.md body — contains AGENTS Guidance is applied rule', () => {
  assert.ok(planWriterBody.includes('AGENTS Guidance is applied'), 'body must contain AGENTS Guidance is applied rule');
});
...
test('qrspi-task-spec-writer.md body — contains Status — PASS and Status — FAIL return format', () => {
  assert.ok(taskSpecWriterBody.includes('### Status — PASS'), 'body must contain PASS return format');
  assert.ok(taskSpecWriterBody.includes('### Status — FAIL'), 'body must contain FAIL return format');
});

test('qrspi-task-spec-writer.md body — contains required reads list', () => {
  assert.ok(taskSpecWriterBody.includes('goals.md'), 'body must reference goals.md in required reads');
  assert.ok(taskSpecWriterBody.includes('requirements.md'), 'body must reference requirements.md in required reads');
  assert.ok(taskSpecWriterBody.includes('research/summary.md'), 'body must reference research/summary.md in required reads');
  assert.ok(taskSpecWriterBody.includes('plan.md'), 'body must reference plan.md in required reads');
  assert.ok(taskSpecWriterBody.includes('phase-manifest.md'), 'body must reference phase-manifest.md in required reads');
});
...
test('qrspi-task-spec-writer.md body — contains Do not invent constraint', () => {
  assert.ok(taskSpecWriterBody.includes('Do not invent'), 'body must contain Do not invent constraint');
});

test('qrspi-task-spec-writer.md body — contains Files paths must come from approved sources only', () => {
  assert.ok(
    taskSpecWriterBody.includes('from approved sources only') || taskSpecWriterBody.includes('approved sources'),
...
test('qrspi-task-spec-reviewer.md body — contains ### Mutations Applied output section', () => {
  assert.ok(taskSpecReviewerBody.includes('### Mutations Applied'), 'body must contain Mutations Applied output section');
});

test('qrspi-task-spec-reviewer.md body — does not contain instructions to edit task files in place', () => {
  assert.ok(!taskSpecReviewerBody.includes('edit task'), 'body must not instruct editing task files in place');
});
...
test('qrspi-task-spec-reviewer.md body — contains ### Review Findings table', () => {
  assert.ok(taskSpecReviewerBody.includes('### Review Findings'), 'body must contain Review Findings table');
});

test('qrspi-task-spec-reviewer.md body — contains ### Unresolved Cross-Task Conflicts section', () => {
  assert.ok(taskSpecReviewerBody.includes('### Unresolved Cross-Task Conflicts'), 'body must contain Unresolved Cross-Task Conflicts section');
});
...
test('qrspi-task-spec-reviewer.md body — contains the 10 review areas', () => {
  ...
  for (const area of reviewAreas) {
    assert.ok(taskSpecReviewerBody.includes(area), `Review Findings must include: ${area}`);
  }
});
...
test('qrspi-plan-reviewer.md body — contains ### Review Findings with 16 review areas', () => {
  ...
  for (const area of reviewAreas) {
    assert.ok(planReviewerBody.includes(area), `Review Findings must include: ${area}`);
  }
});

test('qrspi-plan-reviewer.md body — contains ### Fix Guidance, ### Weakest Areas, and ### Summary', () => {
  assert.ok(planReviewerBody.includes('### Fix Guidance'), 'body must contain Fix Guidance section');
  assert.ok(planReviewerBody.includes('### Weakest Areas'), 'body must contain Weakest Areas section');
  assert.ok(planReviewerBody.includes('### Summary'), 'body must contain Summary section');
});

test('qrspi-plan-reviewer.md body — contains worked examples', () => {
  assert.ok(planReviewerBody.includes('Good review'), 'body must contain good review example');
  assert.ok(planReviewerBody.includes('Bad review'), 'body must contain bad review example');
});
...
test('qrspi-plan-reviewer.md body — contains red flags for key defect categories', () => {
  assert.ok(planReviewerBody.includes('forward'), 'body must flag forward dependencies');
  assert.ok(planReviewerBody.includes('placeholder') || planReviewerBody.includes('TBD'), 'body must flag placeholders');
  assert.ok(planReviewerBody.includes('quick-fix'), 'body must flag quick-fix cardinality issues');
  assert.ok(planReviewerBody.includes('acceptance criterion'), 'body must flag missing acceptance criteria');
});
...
test('qrspi-baseline-checker.md body — does not contain @build subagent dispatch reference', () => {
  assert.ok(!baselineCheckerBody.includes('@build'), 'body must not contain @build subagent reference');
});

test('qrspi-baseline-checker.md body — contains instructions to discover commands from package.json using bash', () => {
  assert.ok(baselineCheckerBody.includes('package.json'), 'body must reference package.json');
  assert.ok(baselineCheckerBody.includes('bash'), 'body must reference bash for command execution');
});

test('qrspi-baseline-checker.md body — contains the four status categories', () => {
  assert.ok(baselineCheckerBody.includes('PASS'), 'body must contain PASS status');
  assert.ok(baselineCheckerBody.includes('FAIL'), 'body must contain FAIL status');
  assert.ok(baselineCheckerBody.includes('NOT CONFIGURED'), 'body must contain NOT CONFIGURED status');
  assert.ok(baselineCheckerBody.includes('SKIPPED'), 'body must contain SKIPPED status');
});
...
test('qrspi-baseline-checker.md body — contains ### Baseline Status — CLEAN or DIRTY', () => {
  assert.ok(baselineCheckerBody.includes('### Baseline Status'), 'body must contain Baseline Status header');
  assert.ok(baselineCheckerBody.includes('CLEAN'), 'body must reference CLEAN status');
  assert.ok(baselineCheckerBody.includes('DIRTY'), 'body must reference DIRTY status');
});

test('qrspi-baseline-checker.md body — contains ### Check Results, ### Failure Inventory, and ### Stage Summary', () => {
  assert.ok(baselineCheckerBody.includes('### Check Results'), 'body must contain Check Results table');
  assert.ok(baselineCheckerBody.includes('### Failure Inventory'), 'body must contain Failure Inventory table');
  assert.ok(baselineCheckerBody.includes('### Stage Summary'), 'body must contain Stage Summary section');
});
...
test('All 6 agents — body is non-empty', () => {
  ...
  assert.ok(planWriterBody.length > 0, 'qrspi-plan-writer body must be non-empty');
  assert.ok(taskSpecWriterBody.length > 0, 'qrspi-task-spec-writer body must be non-empty');
  assert.ok(taskSpecReviewerBody.length > 0, 'qrspi-task-spec-reviewer body must be non-empty');
  assert.ok(planReviewerBody.length > 0, 'qrspi-plan-reviewer body must be non-empty');
  assert.ok(baselineCheckerBody.length > 0, 'qrspi-baseline-checker body must be non-empty');
});
