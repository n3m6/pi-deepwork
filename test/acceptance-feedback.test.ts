...
import { renderAcceptanceRepairContext } from "../src/stages/acceptance-feedback.js";
import { runFastImplCodeSubstage } from "../src/stages/fast-impl-code.js";
import { runFastImplTestSubstage } from "../src/stages/fast-impl-test.js";
import { createInitialState, ensureRunDirectories, getRunArtifacts } from "../src/state.js";
import type { DispatchRequest, DispatchResult, Dispatcher, PipelineServices, StageRuntime } from "../src/types.js";
...
test("renderAcceptanceRepairContext returns empty string when acceptFixAttempts is 0", async () => {
  ...
  const services = { commandContext: { signal: undefined } } as unknown as PipelineServices;
...
test("runFastImplCodeSubstage embeds repair context in prompt when acceptFixAttempts > 0", async () => {
  ...
  const taskSpecPath = path.join(harness.artifacts.tasksDir, "task-01.md");
  await writeFile(
    taskSpecPath,
  ...
  await runFastImplCodeSubstage(
    ...
    { taskId: "01", worktreeRoot: harness.workspaceRoot, taskSpecPath, attempt: 2 },
  );
...
test("runFastImplCodeSubstage does not embed repair context when acceptFixAttempts is 0", async () => {
  ...
  await runFastImplCodeSubstage(
    ...
    { taskId: "01", worktreeRoot: harness.workspaceRoot, taskSpecPath, attempt: 1 },
  );
