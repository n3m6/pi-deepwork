...
import { runFastImplLoopSubstage } from "../src/stages/fast-impl-loop.js";
...
async function writeTaskSpec(harness: TestHarness, taskNumber = "01"): Promise<string> {
  const taskSpecPath = path.join(harness.artifacts.tasksDir, `task-${taskNumber}.md`);
  await writeFile(
    taskSpecPath,
...
test("fast impl loop returns PASS when all steps succeed on attempt 1", async () => {
  ...
  const taskSpecPath = await writeTaskSpec(harness);
  ...
  const result = await runFastImplLoopSubstage(
    ...
    { taskId: "01", worktreeRoot: harness.workspaceRoot, taskSpecPath },
  );
