...
test('qrspi-goals.md body — uses qrspi_dispatch (not task) for subagent dispatch', () => {
  ...
  assert.ok(!/via\s+task\b/i.test(orchBody), 'body must not use "via task" as dispatch reference');
});
...
test('qrspi-goals.md body — uses Read not cat for artifact reads', () => {
  assert.ok(orchBody.includes('Read .pipeline/'), 'body must contain "Read .pipeline/"');
  assert.ok(!orchBody.includes('cat .pipeline/'), 'body must not contain "cat .pipeline/"');
});
...
test('qrspi-goals.md body — contains Step A0 (Preserve Initial Requirements)', () => {
  ...
  assert.ok(orchBody.includes('requirements.md'), 'body must reference requirements.md in Step A0');
});
...
test('qrspi-goals.md body — contains Step C (Write Artifacts)', () => {
  ...
  assert.ok(orchBody.includes('goals.md'), 'body must reference goals.md in Step C');
  assert.ok(orchBody.includes('config.md'), 'body must reference config.md in Step C');
});
...
test('qrspi-goals.md body — contains Step D (Checklist Review Loop) with 5-round cap', () => {
  ...
  assert.ok(orchBody.includes('goals-reviewer'), 'body must reference goals-reviewer in Step D');
...
test('qrspi-goals.md body — Step D three-way loop decision text is fully specified', () => {
  ...
  assert.ok(orchBody.includes('review_round < 5'), 'Step D must specify re-dispatch condition (< 5)');
  assert.ok(orchBody.includes('review_round = 5'), 'Step D must specify cap hit condition (= 5)');
...
test('qrspi-goals.md body — contains Step E (Human Gate) with feedback loop', () => {
  ...
  assert.ok(orchBody.includes('approve'), 'body must mention approve flow in Step E');
  assert.ok(orchBody.includes('feedback'), 'body must reference feedback handling in Step E');
});

test('qrspi-goals.md body — Step E feedback loop includes full orchestration substeps', () => {
  // Verify feedback file writing paths
  assert.ok(orchBody.includes('feedback/goals-round-'), 'Step E must specify feedback file path with goals-round-{NN}.md');
  assert.ok(orchBody.includes('mkdir -p') && orchBody.includes('feedback'), 'Step E must create feedback directory');
  // Verify requirements.md rebuild
  assert.ok(orchBody.includes('requirements.md'), 'Step E must reference rebuilding requirements.md');
  // Verify FEEDBACK HISTORY for re-dispatch
  assert.ok(orchBody.includes('FEEDBACK HISTORY'), 'Step E must include FEEDBACK HISTORY section for synthesizer re-dispatch');
  // Verify review_round reset after feedback
  assert.ok(orchBody.includes('review_round = 1') || orchBody.includes('review_round=1'), 'Step E must reset review_round to 1 after feedback');
  // Verify re-dispatch of synthesizer
  assert.ok(orchBody.includes('qrspi-goals-synthesizer'), 'Step E must re-dispatch qrspi-goals-synthesizer on feedback');
});

test('qrspi-goals.md body — Step E pre-condition check guards against unclean-cap', () => {
  assert.ok(orchBody.includes('unclean-cap'), 'body must contain unclean-cap guard term');
  assert.ok(orchBody.includes('Pre-condition check'), 'body must contain pre-condition check directive');
  assert.ok(orchBody.includes('### Status — FAIL'), 'body must contain unrecoverable FAIL return template');
});
...
test('qrspi-goals.md body — Step E unclean-cap skip logic uses unrecoverable-failure template', () => {
  ...
  assert.ok(
    orchBody.includes('unclean-cap') && orchBody.includes('skip this step entirely'),
    'unclean-cap pre-condition must connect to skip directive'
  );
});

test('qrspi-goals.md body — contains Critical Rule directives', () => {
  assert.ok(orchBody.includes('No code edits'), 'body must contain Critical Rule 1: No code edits');
  assert.ok(orchBody.includes('End your turn'), 'body must contain Critical Rule 3: End your turn');
  assert.ok(orchBody.includes('Dispatch subagents directly'), 'body must contain Critical Rule 2: Dispatch subagents directly');
});

test('qrspi-goals.md body — Critical Rules include full sub-instruction text', () => {
  // Rule 1: No code edits + pipeline file restriction
  assert.ok(
    orchBody.includes('No code edits') && orchBody.includes('Write only pipeline state files'),
  ...
  assert.ok(
    orchBody.includes('Dispatch subagents directly') && orchBody.includes('Never describe a handoff in plain text'),
  ...
  assert.ok(
    orchBody.includes('Stop after each subagent dispatch') && orchBody.includes('End your turn') && orchBody.includes('wait for the response'),
...
test('qrspi-goals.md body — contains core behavioral rules', () => {
  ...
  for (const branch of coverageBranches) {
    assert.ok(orchBody.includes(branch), `body must contain coverage branch: ${branch}`);
  }
  ...
  assert.ok(
    orchBody.includes('Scope decomposition') || orchBody.includes('scope decomposition'),
    'body must contain scope-decomposition trigger (A4.4)'
  );

  assert.ok(
    orchBody.includes('Pre-condition check') && orchBody.includes('unclean-cap'),
...
test('qrspi-goals-synthesizer.md body — specifies output sections ### goals.md and ### config.md', () => {
  assert.ok(synthBody.includes('### goals.md'), 'body must contain ### goals.md output section');
  assert.ok(synthBody.includes('### config.md'), 'body must contain ### config.md output section');
});

test('qrspi-goals-synthesizer.md body — goals.md structure contains all required sections', () => {
  assert.ok(synthBody.includes('## Intent'), 'body must specify Intent section');
  assert.ok(synthBody.includes('## Functional Requirements'), 'body must specify Functional Requirements section');
  assert.ok(synthBody.includes('## Non-Functional Requirements'), 'body must specify Non-Functional Requirements section');
  assert.ok(synthBody.includes('## Technical Specification'), 'body must specify Technical Specification section');
  assert.ok(synthBody.includes('## Constraints'), 'body must specify Constraints section');
  assert.ok(synthBody.includes('## Non-Goals'), 'body must specify Non-Goals section');
  assert.ok(synthBody.includes('## Acceptance Criteria'), 'body must specify Acceptance Criteria section');
});
...
test('qrspi-goals-synthesizer.md body — config.md structure contains required and optional fields', () => {
  ...
  assert.ok(synthBody.includes('coverage_threshold:'), 'body must reference optional coverage_threshold');
  assert.ok(synthBody.includes('test_globs:'), 'body must reference optional test_globs');
});

test('qrspi-goals-synthesizer.md body — source authority: user-answer and user-confirmed-finding are authoritative', () => {
  assert.ok(
    synthBody.includes('user-answer') && synthBody.includes('user-confirmed-finding'),
...
test('qrspi-goals-synthesizer.md body — repo-finding must not appear in FR, Constraints, AC', () => {
  assert.ok(synthBody.includes('repo-finding'), 'body must reference repo-finding');
...
test('qrspi-goals-reviewer.md body — specifies output sections', () => {
  assert.ok(reviewBody.includes('### Status — PASS or FAIL'), 'body must contain Status header');
  assert.ok(reviewBody.includes('### Review Findings'), 'body must contain Review Findings header');
  assert.ok(reviewBody.includes('### Fix Guidance'), 'body must contain Fix Guidance header');
...
test('qrspi-goals-reviewer.md body — Review Findings contains 9 check areas', () => {
  ...
  for (const area of checkAreas) {
    assert.ok(reviewBody.includes(area), `Review Findings must include: ${area}`);
  }
});
...
test('qrspi-goals-reviewer.md body — Inference integrity check traces to user-answer/user-confirmed-finding', () => {
  ...
  assert.ok(reviewBody.includes('Inference integrity'), 'body must contain Inference integrity check');
  assert.ok(
    reviewBody.includes('user-answer') || reviewBody.includes('user-confirmed-finding'),
...
test('Agents — no opencode permission system references in any agent file', () => {
  ...
  const allBodies = { orchestrator: orchBody, synthesizer: synthBody, reviewer: reviewBody };
...
test('Agents — all three agent files exist and have parseable frontmatter', () => {
  ...
  assert.ok(synthFM !== null, 'qrspi-goals-synthesizer.md has parseable frontmatter');
  assert.ok(reviewFM !== null, 'qrspi-goals-reviewer.md has parseable frontmatter');
});
