...
test("instrumentCustomTools records calls in sink and invokes callback", async () => {
  const sink: import("../src/types.js").DispatchCustomToolCall[] = [];
...
test("existingPaths keeps real paths", async () => {
  ...
  const existing = await existingPaths([join(process.cwd(), "src/dispatch.ts")]);
...
test("dispatchChain substitutes {previous} in subsequent prompts", async () => {
  ...
  const factory = async (request: DispatchRequest): Promise<import("../src/dispatch.js").AgentSession> => {
