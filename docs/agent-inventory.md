...
This inventory is the cutover checklist for the deterministic TypeScript migration.

- `Delete` means the agent carries orchestration logic (`tools: subagent`) that is ported into `src/`.
- `Keep` means the file remains a markdown prompt payload dispatched by the runtime.
- The old `general-purpose` worker was never bundled in this repo, so it is not listed here. Its role becomes a plain pi coding session dispatched by the runtime.

| Agent | Stage / Area | Current Role | Cutover | Replacement |
| --- | --- | --- | --- | --- |
| `qrspi-accept` | Stage 7 | Stage orchestrator | Delete | `src/stages/accept.ts` |
| `qrspi-acceptance-tester` | Stage 7 | Acceptance sub-orchestrator | Delete | `src/stages/acceptance-tester.ts` |
...
| `qrspi-baseline-regression-checker` | Stage 6 | Baseline regression orchestrator | Delete | `src/stages/baseline-regression.ts` |
| `qrspi-code-review` | Stage 6 | Review fanout sub-orchestrator | Delete | `src/stages/code-review.ts` |
...
| `qrspi-design` | Stage 3 | Stage orchestrator | Delete | `src/stages/design.ts` |
...
| `qrspi-e2e-regression-checker` | Stage 6 | E2E regression orchestrator | Delete | `src/stages/e2e-regression.ts` |
| `qrspi-fast-impl-code` | Stage 6 | TDD inner-loop orchestrator | Delete | `src/stages/fast-impl-code.ts` |
| `qrspi-fast-impl-loop` | Stage 6 | TDD inner-loop orchestrator | Delete | `src/stages/fast-impl-loop.ts` |
| `qrspi-fast-impl-test` | Stage 6 | TDD inner-loop orchestrator | Delete | `src/stages/fast-impl-test.ts` |
| `qrspi-fast-impl-verify` | Stage 6 | TDD inner-loop orchestrator | Delete | `src/stages/fast-impl-verify.ts` |
| `qrspi-goals` | Stage 1 | Stage orchestrator | Delete | `src/stages/goals.ts` |
...
| `qrspi-implement` | Stage 6 | Stage orchestrator | Delete | `src/stages/implement.ts` |
| `qrspi-integration-checker` | Stage 6 | Integration check leaf | Keep | Markdown prompt |
| `qrspi-plan` | Stage 5 | Stage orchestrator | Delete | `src/stages/plan.ts` |
...
| `qrspi-questions` | Stage 2 | Question-batch orchestrator | Delete | `src/stages/questions.ts` |
| `qrspi-report` | Stage 10 | Stage orchestrator | Delete | `src/stages/report.ts` |
| `qrspi-replan` | Stage 8 | Stage orchestrator | Delete | `src/stages/replan.ts` |
...
| `qrspi-research` | Stage 2 | Stage orchestrator | Delete | `src/stages/research.ts` |
| `qrspi-research-pass` | Stage 2 | Research-pass orchestrator | Delete | `src/stages/research-pass.ts` |
...
| `qrspi-structure` | Stage 4 | Stage orchestrator | Delete | `src/stages/structure.ts` |
...
| `qrspi-verify` | Stage 9 | Stage orchestrator | Delete | `src/stages/verify.ts` |
