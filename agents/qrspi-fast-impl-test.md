...
12. **Repair Context** — on `test-repair`: `### Route Context` block from `qrspi-fast-impl-verify`. On `test-sync`: `None.` in fresh mode, or regression evidence in fix mode.
...
**Step 0 — Testability.** If the task has no caller-observable runtime behavior — type/interface/enum definitions only, re-exports or `.d.ts` files, configuration, documentation, or empty scaffolding — return the `NO_TASK_AUTHORED_TESTS` outcome immediately without dispatching the `general-purpose` child worker.
...
**Step 2 — Classify.** Run each candidate at least twice in isolation via the `general-purpose` child worker. Assign one class from Evidence Classes above; cite the basis.
...
**Step 7 — Validate.** Run all adopted, repaired, and written tests via the `general-purpose` child worker. Reclassify inconsistent tests as FLAKY and move to Unsafe Evidence before returning. Tests that fail to load with an import error, missing-symbol error, or syntax error from an API change → classify as `HARNESS_NOISY` with `Reason: references symbol/import not in current codebase` (no behavior was actually exercised, so the failure is not task-relevant; the verifier's `TEST_REPAIR` routing and the per-task code-review's test-quality reviewer adjudicate repair vs. delete downstream).
