...
2. **DIRECT CHILD DISPATCH.** Invoke all child agents with `subagent`. For single-child work, use `subagent({ agent: "...", context: "fresh", task: `...` })` and use the returned subagent result directly.
3. **WAIT BEFORE CONTINUING.** After each child call returns, use that returned result before dispatching the next child.
...
Call `subagent` for CODE → TEST → VERIFY in sequence (use each returned result before dispatching the next). After each child return, if FAIL or `### Backward Loop Request`: stop and return immediately (see **Return**).

For each child dispatch use `subagent` with the pattern:

```
subagent({
  agent: "qrspi-fast-impl-code",   // or "qrspi-fast-impl-test" / "qrspi-fast-impl-verify"
  context: "fresh",
  task: `<BASE CONTEXT + child-specific sections>`
})
```

Use the returned subagent result directly.

**Fresh mode:**

- CODE (`agent: "qrspi-fast-impl-code"`): entry_type=`fresh`, repair_context=`None.`, instructions: `Implement the production code required by this task. No test files. Max 3 iterations.`
- TEST (`agent: "qrspi-fast-impl-test"`): entry_type=`test-sync`, repair_context=`None.`, fix_mode=`no`, instructions: `Discover, classify, adopt, repair, and write tests. Max 3 iterations. Return the authoritative evidence-classified test inventory.`
- VERIFY (`agent: "qrspi-fast-impl-verify"`): prior_verify_result=`None.`, regression_evidence=`None.`, instructions: `Run targeted verification, dispatch qrspi-code-review, apply safe local fixes within 2 review rounds, and commit only on CLEAN success. Return an explicit Route Hint.`
