...
3. **Launch the full review batch before joining.** Start the selected reviewers with one `subagent({ context: "fresh", tasks: [...] })` call and use the returned batch results before collating.
...
Launch the selected reviewers with one `subagent({ context: "fresh", tasks: [...] })` call. Use the returned batch results as the reviewer results in request order. Send each reviewer:
