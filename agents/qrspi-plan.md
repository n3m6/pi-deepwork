...
- Stop after each subagent call returns and use that returned result before moving to the next step.
...
Call `subagent` for `qrspi-plan-writer`. For **quick-fix**, omit the `=== DESIGN ===` and `=== STRUCTURE ===` fields.
...
Set `review_round = 1`. For each round, call `subagent` for `qrspi-plan-reviewer`:
