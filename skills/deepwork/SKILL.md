...
4. **CHECK YOUR SUBAGENT INVENTORY** Before dispatching a stage, verify the expected `qrspi-*` subagent is present in the `subagent list`. If it is missing, **call the `deepwork_bootstrap` tool first** (the pi-deepwork extension owns `<workspace>/.pi/agents/` and exposes this tool to mirror the bundled qrspi agents idempotently). If the tool returns `status=error` or `ok=false`, **stop the run**: emit a `run.failed` telemetry event with `reason: "missing_subagent"` and `subagent: <name>`, then report `Deepwork configuration error: missing subagent <name>. The deepwork_bootstrap tool returned: <error>. Run /deepwork-doctor for full diagnostics and read .pi/deepwork-doctor-report.md.` **Do not manually mirror, symlink, copy, or otherwise touch `<workspace>/.pi/agents/`** — call the `deepwork_bootstrap` tool instead. Any manual interference will be overwritten on the next session.
...
0. **Bootstrap the runtime** by calling the `deepwork_bootstrap` tool with the workspace cwd. This mirrors the bundled `qrspi-*` subagents into `<workspace>/.pi/agents/` and refreshes the pi-subagents registry. The tool is idempotent — calling it on an already-mirrored workspace is a no-op. If the tool returns `status=error` or `ok=false`, **stop the run**: emit a `run.failed` telemetry event with `reason: "bootstrap_failed"` and the error message, then report `Deepwork bootstrap error: <error>. Run /deepwork-doctor and read .pi/deepwork-doctor-report.md for diagnostics.` Do NOT attempt to mirror `.pi/agents/` manually as a fallback — the extension owns that directory.
