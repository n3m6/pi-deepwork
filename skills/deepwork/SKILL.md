...
- `interaction_mode: interactive` — ask the user at human gates and error gates using `ask_user`. Forward inbound child intercom asks to the user (see Critical Rule 7).
- `interaction_mode: automated` — do not use `ask_user` for gates that have a deterministic policy below. Do not use `contact_supervisor` in child subagents either. Record synthetic gate telemetry with `context.gate_mode: "automated"` and the automatic decision.
