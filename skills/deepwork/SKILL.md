...
4. **CHECK YOUR SUBAGENT INVENTORY** Before dispatching a stage, verify the expected `qrspi-*` subagent is present in the `subagent list`. If it is missing, report `Deepwork configuration error: missing subagent <name>` and stop without dispatching.
